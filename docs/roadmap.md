...
- **CLI**: `agentkit/cli.py` → `init | sync | validate | upgrade`, plus
  `presets list/add`, `skills list`, `manifest new`, and
  `agents generate --provider codex`.
...
- **Templates** shipped to projects: `agentkit/templates/` (AGENTS.base, base MCP =
  context7+filesystem, docs taxonomy, hooks, schemas).
- **Bundled presets**: `preset-workflow-standard`, `preset-spring-boot`
  (minimal), `preset-angular`.
- **Tests**: `tests/` — engine units, init integration incl. non-code project +
  memory, preset apply, CLI UX, validation, upgrade incl. core-region merge +
  preset refresh.
...
- Vocabulary / layers: core engine + `workflow-*` / `stack-*` project skills.
...
- Core stays minimal & general-purpose (code, non-code, research, general use);
  workflow/stack skills are opt-in via presets.
- Preset metadata is `preset.json`; the user manifest is `agentkit.yaml`.
- Docs organized by audience (using / authoring presets / developing core) and by
  artifact type.
...
3. **Provider delegation expansion** after provider-specific custom-agent formats
   are verified for Claude/Kiro.
...
(`agentkit upgrade --refresh-presets`); standard workflow preset; Codex-only
