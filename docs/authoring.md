...
2. Pick the layer prefix: `workflow-*` (reusable task workflow) or `stack-*`
...
top of core. It must not modify core-managed files (the engine and the governance
region of `AGENTS.md`). Applying a preset is **additive and
...
```
preset-<stack>/
  preset.json          name, version, coreVersion, kind (workflow|stack), language, framework
  skills/<name>/SKILL.md   skills to add (workflow-* or stack-*), one level deep
